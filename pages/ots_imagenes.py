...
def main():
    ...
    st.markdown("<h1>"+"Imágenes OT #"+"12345"+"</h1>", unsafe_allow_html=True)
    ct.sidebar()
    ...
    col1.dataframe(pd.DataFrame({"Patente":["AB1234"]}),use_container_width=True,hide_index=True)

    
    # insert_img = col2.button(label="Agregar Imágenes",key="a2", type="primary",icon=":material/add:")
    col2.subheader("Imágenes cargadas")
    ...
    col4.subheader("Subir Imágen")
    with col4.container(height=600):
        ...
        if imgs is not None:
            if len(imgs) > 10 - total_cargadas:
                ...
                colc.button("Subir Imágenes",disabled=True,icon=":material/upload:")
                ...
                if colc.button("Subir Imágenes",type="primary",icon=":material/upload:"):
